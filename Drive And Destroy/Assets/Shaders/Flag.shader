    ...
    SubShader {
        ...
        Pass {
            ...
            UNITY_INSTANCING_BUFFER_START( Props )
                ...
                UNITY_DEFINE_INSTANCED_PROP( float, _XStrength)
                UNITY_DEFINE_INSTANCED_PROP( float, _Constrainsize)
                UNITY_DEFINE_INSTANCED_PROP( float, _ZStrength)
                UNITY_DEFINE_INSTANCED_PROP( float, _YStrength)
                UNITY_DEFINE_INSTANCED_PROP( float, _WindSpeed)
                UNITY_DEFINE_INSTANCED_PROP( float, _Selector)
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float node_4144 = 0.1;
                float node_9856_if_leA = step(_WindSpeed_var,node_4144);
                float node_9856_if_leB = step(node_4144,_WindSpeed_var);
                ...
                float node_5208 = (node_2869.g*_WindSpeed_var);
                float node_5933 = abs((frac(((node_3240*3.0)+node_5208*float2(0.5,0)).r)-0.5));
                float _XStrength_var = UNITY_ACCESS_INSTANCED_PROP( Props, _XStrength );
                float node_9243 = abs((frac(((node_3240*2.0)+node_5208*float2(0.5,0)).r)-0.5));
                float _YStrength_var = UNITY_ACCESS_INSTANCED_PROP( Props, _YStrength );
                float _ZStrength_var = UNITY_ACCESS_INSTANCED_PROP( Props, _ZStrength );
                float node_3166 = (o.uv0.r*1.0);
                float _Constrainsize_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Constrainsize );
                float node_3845 = abs((frac(o.uv0.g)-0.5));
                float node_1203 = 0.6;
                v.vertex.xyz += saturate(min(float3(saturate((saturate(( node_5933 > 0.5 ? (1.0-(1.0-2.0*(node_5933-0.5))*(1.0-lerp(0.3,0.7,(node_1108+node_5933)))) : (2.0*node_5933*lerp(0.3,0.7,(node_1108+node_5933))) ))*_XStrength_var)),saturate((node_9243*_YStrength_var)),saturate(((abs((frac(((o.uv0*0.5)+node_5208*float2(0.5,0)).r)-0.5))*0.2)*_ZStrength_var))),(pow((node_3166+_Constrainsize_var),25.0)+(1.0 - (node_3845+node_1203)))));
...
////// Specular:
                ...
                float _Selector_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Selector );
                float node_901_if_leA = step(1.0,floor(_Selector_var));
                float node_901_if_leB = step(floor(_Selector_var),1.0);
                ...
                float3 diffuseColor = (lerp(_Color1_var.rgb,_Color2_var.rgb,lerp((node_901_if_leA*_MainTex_var.b)+(node_901_if_leB*_MainTex_var.r),_MainTex_var.g,node_901_if_leA*node_901_if_leB))*_Multiply_var); // Need this for specular when using metallic
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : SV_Target {
                ...
                float4 _MainTex_var = tex2D(_MainTex,TRANSFORM_TEX(i.uv1, _MainTex));
                float _Multiply_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Multiply );
                float3 diffColor = (lerp(_Color1_var.rgb,_Color2_var.rgb,lerp((node_901_if_leA*_MainTex_var.b)+(node_901_if_leB*_MainTex_var.r),_MainTex_var.g,node_901_if_leA*node_901_if_leB))*_Multiply_var);
                float specularMonochrome;
                float3 specColor;
                float _Metallic_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Metallic );
