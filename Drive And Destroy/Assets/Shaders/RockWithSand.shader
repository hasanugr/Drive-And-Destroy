    ...
    SubShader {
        ...
        Pass {
            ...
            UNITY_INSTANCING_BUFFER_START( Props )
                ...
                UNITY_DEFINE_INSTANCED_PROP( float, _NoiseHardness)
                UNITY_DEFINE_INSTANCED_PROP( float, _Dustroughness)
...
///////// Gloss:
                ...
                float _Dustroughness_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Dustroughness );
                float _Amount_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Amount );
                float _Hardness_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Hardness );
                float _NoiseScale_var = UNITY_ACCESS_INSTANCED_PROP( Props, _NoiseScale );
                float2 node_1677 = (i.uv0*_NoiseScale_var);
                ...
                float _NoiseHardness_var = UNITY_ACCESS_INSTANCED_PROP( Props, _NoiseHardness );
                float Mask = saturate(((saturate((pow(normalDirection.g,(_Amount_var*-1.0+0.0))*_Hardness_var))*saturate((((_NoiseMap_var.r+_NoiseAmount_var)*6.0+-5.0)*_NoiseHardness_var)))-(_BumpMap_var.r*0.5)));
                float gloss = 1.0 - lerp(_RockRMA_var.r,_Dustroughness_var,Mask); // Convert roughness to gloss
                float perceptualRoughness = lerp(_RockRMA_var.r,_Dustroughness_var,Mask);
...
////// Specular:
                ...
                float3 diffuseColor = lerp(lerp(node_329,(node_329*_RockRMA_var.b),_AOAmount_var),_DustAlbedo_var.rgb,Mask); // Need this for specular when using metallic
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float3 _BumpMap_var = UnpackNormal(tex2D(_BumpMap,TRANSFORM_TEX(i.uv0, _BumpMap)));
                float Mask = saturate(((saturate((pow(normalDirection.g,(_Amount_var*-1.0+0.0))*_Hardness_var))*saturate((((_NoiseMap_var.r+_NoiseAmount_var)*6.0+-5.0)*_NoiseHardness_var)))-(_BumpMap_var.r*0.5)));
                float3 diffColor = lerp(lerp(node_329,(node_329*_RockRMA_var.b),_AOAmount_var),_DustAlbedo_var.rgb,Mask);
                float specularMonochrome;
                float3 specColor;
                diffColor = DiffuseAndSpecularFromMetallic( diffColor, _RockRMA_var.g, specColor, specularMonochrome );
                float _Dustroughness_var = UNITY_ACCESS_INSTANCED_PROP( Props, _Dustroughness );
                float roughness = lerp(_RockRMA_var.r,_Dustroughness_var,Mask);
